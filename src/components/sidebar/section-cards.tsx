...
import { useSuspenseDashboardData } from "@/data/admin/hooks/use-admin"

export   function SectionCards() {
  const { data:{totalCourses,totalLessons,totalSignUps,totalCustomers} } = useSuspenseDashboardData()
  return (
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs  @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      <Card className="@container/card">
        <CardHeader  className="flex-row justify-between flex items-center space-y-0 pb-2">
          <div>
